...
public static class Tf2Ps3SourceLoadingReplacementPlanReducer
{
        ...
        string outputPath)
    {
        ...
        var acceptanceBlockingCount = blockers.Sum(static blocker => blocker.BlockingCount);
        var steadyPaddingRisk = ReadInt(loadingSummary, "SteadyNativeSnapshotPaddingRiskCount");
        var nativeMapLoadReady = acceptanceBlockingCount == 0;
        var report = new Tf2Ps3SourceLoadingReplacementPlanReport(
            ...
            new Tf2Ps3SourceLoadingReplacementPlanSummary(
                ...
                ReadInt(loadingSummary, "NativeRecordPrefixDebtBytes"),
                ReadInt(loadingSummary, "NativeRecordTrailingDebtBytes"),
                ReadInt(loadingSummary, "NativeRecordVisibleRecordBytes"),
            ...
            [
                ...
                steadyPaddingRisk == 0
                    ? "Replace high-entropy snapshot fallback after the bootstrap path: generate SVC_PacketEntities and entity-delta payloads from field writers while keeping steady snapshots unpadded."
            ...
            [
                nativeMapLoadReady
                    ? "The current map-load gate is no longer blocked by unknown Plasma/GameManager text or generated loading/map-load Source payload debt."
                ...
                nativeMapLoadReady
                    ? "TF.elf writer evidence is now represented by native object-stream/bootstrap/snapshot field contracts in the live responder."
                ...
                nativeMapLoadReady
                    ? "This report intentionally keeps acceptance strict: production fake frames, generated queued-prefix frames, generated native-record prefixes, steady padding risk, and static templates are all zero."
                    : steadyPaddingRisk == 0
                    ? "This report intentionally keeps acceptance strict: the server is not native while any production fake frame, generated queued-prefix frame, or static template remains."
    ...
    private static IEnumerable<Tf2Ps3SourceLoadingReplacementTrack> BuildTracks(Tf2Ps3SourceLoadingReplacementTrackInputs inputs)
    {
        ...
        yield return new Tf2Ps3SourceLoadingReplacementTrack(
            ...
            [
                ...
                "For the second initial object-stream record, append the 00a56cb0 server-object CBaseClient::SendSignonData/m_Server->m_Signon buffer from +0xd8/+0xe4 after SVC_ClassInfo and before NET_SignonState state 4.",
            ...
            [
                ...
                $"Object stream bootstrap report reduces {ReadInt(inputs.ObjectSummary, "InitialBootstrapBatchCount")} initial kind-1 batch(es), {ReadInt(inputs.ObjectSummary, "InitialBootstrapRawAppendCount")} initial raw append, and {ReadInt(inputs.ObjectSummary, "PostBootstrapRawAppendCount")} kind-2 update raw append(s).",
        ...
        var generatedQueuedDebt = inputs.GeneratedPrefixDebts
            .Where(static debt => debt.ReplacementTarget == "queued-peer-submessage-boundaries")
            .ToArray();
        var steadyPaddingRisk = ReadInt(inputs.LoadingSummary, "SteadyNativeSnapshotPaddingRiskCount");
        yield return new Tf2Ps3SourceLoadingReplacementTrack(
            "queued-peer-submessage-boundaries",
            "open-native-implementation-required",
            "008bc978 -> 008b9f70/008bb058/008bc490",
            new Tf2Ps3SourceLoadingReplacementDebt(
                "mixed-binary filler and PNG/COc/DSC frames with generated prefix/trailing native Source stream bytes",
                ...
                ReadInt(inputs.LoadingSummary, "NativeRecordPrefixDebtBytes") + ReadInt(inputs.LoadingSummary, "NativeRecordTrailingDebtBytes") + generatedQueuedDebt.Sum(static debt => debt.PrefixByteLength),
            ...
            [
                "NativeRecordWithGeneratedPrefixFrameCount, NativeRecordPrefixDebtBytes, and NativeRecordTrailingDebtBytes are zero.",
        ...
        yield return new Tf2Ps3SourceLoadingReplacementTrack(
            ...
            new Tf2Ps3SourceLoadingReplacementDebt(
                steadyPaddingRisk == 0
                    ? "steady native snapshot/entity-delta route has no deterministic padding debt"
                    : "high-entropy filler and steady native snapshot bodies padded to PCAP-shaped lengths",
                steadyPaddingRisk,
                steadyPaddingRisk == 0 ? 0 : ReadInt(inputs.LoadingSummary, "BlockingByteCount"),
                steadyPaddingRisk,
            ...
            [
                ...
                steadyPaddingRisk == 0
                    ? "Keep steady snapshot production on unpadded native snapshot/entity-delta bodies."
            ...
            [
                ...
                steadyPaddingRisk == 0
                    ? "Steady-state frames no longer carry deterministic padding risk in production output."
    ...
    {
        yield return new("production-fake-loading-frames", ReadInt(loadingSummary, "ProductionFakeFrameCount"), "Replace queued-boundary placeholder loading payloads with exact native Source field writers.");
        yield return new("generated-native-record-stream-bytes", ReadInt(loadingSummary, "NativeRecordWithGeneratedPrefixFrameCount"), "Recover queued prefix/trailing Source bitstream bytes around native PNG/COc/DSC records.");
        yield return new("native-record-prefix-debt-bytes", ReadInt(loadingSummary, "NativeRecordPrefixDebtBytes"), "Replace pre-record deterministic bytes with the native TF.elf/server.dll Source field writer.");
        yield return new("native-record-trailing-debt-bytes", ReadInt(loadingSummary, "NativeRecordTrailingDebtBytes"), "Replace post-record deterministic bytes with the native TF.elf/server.dll Source field writer.");
        yield return new("steady-snapshot-padding-risk", ReadInt(loadingSummary, "SteadyNativeSnapshotPaddingRiskCount"), "Keep steady snapshot production routes free of deterministic padding.");
